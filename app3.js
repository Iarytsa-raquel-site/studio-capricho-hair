...
async function acceptBookingProposal(id){
  ...
  if(error){toast(error.message?.includes('HORARIO_INDISPONIVEL')?'Esse horário não está mais disponível.':'Não foi possível aceitar o novo horário.');await syncClientBookings();renderMyBookings();return;}
  await syncClientBookings();renderMyBookings();toast('Novo horário aceito e confirmado!');
}
async function declineBookingProposal(id){
  if(!sb){toast('Conexão indisponível.');return;}
  const b=getBookings().find(x=>x.id===id);if(!b||b.status!=='proposta')return;
  const phone=b.phone||localStorage.getItem('sc_last_phone')||'';
  if(!phone){toast('Não foi possível validar seu agendamento.');return;}
  const {error}=await sb.rpc('decline_booking_proposal',{p_booking_id:id,p_phone:phone});
  if(error){toast('Não foi possível informar ao Studio.');return;}
  await syncClientBookings();renderMyBookings();toast('O Studio foi informado. Aguarde uma nova sugestão.');
}
window.acceptBookingProposal=acceptBookingProposal;window.declineBookingProposal=declineBookingProposal;
