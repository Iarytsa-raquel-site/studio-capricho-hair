...
function performanceData(range=performanceRange){
  const completed=getBookings().filter(b=>b.status==='concluido');
  const now=new Date(),months=[];
  for(let i=range-1;i>=0;i--){const d=new Date(now.getFullYear(),now.getMonth()-i,1);months.push(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`);}
...
function setPerformanceRange(value){performanceRange=Number(value)||6;renderPerformanceDashboard();}
window.setPerformanceRange=setPerformanceRange;
function renderPerformanceDashboard(){
  ...
  <div class="performance-card" style="margin-bottom:16px"><div class="performance-card-head"><div><span>Período de visualização</span><strong>Escolha quantos meses deseja comparar</strong></div><select onchange="setPerformanceRange(this.value)" style="min-width:140px"><option value="3" ${performanceRange===3?'selected':''}>3 meses</option><option value="6" ${performanceRange===6?'selected':''}>6 meses</option><option value="12" ${performanceRange===12?'selected':''}>12 meses</option><option value="24" ${performanceRange===24?'selected':''}>24 meses</option></select></div></div>
  ...
  <div class="performance-card"><div class="performance-card-head"><div><span>Faturamento</span><strong>Últimos ${performanceRange} meses</strong></div><strong>${money(d.byMonth.reduce((s,x)=>s+x.revenue,0))}</strong></div><div class="perf-chart">${d.byMonth.map(x=>`<div class="perf-col"><div class="perf-value">${x.revenue?money(x.revenue).replace(',00',''):''}</div><div class="perf-bar-wrap"><div class="perf-bar revenue" style="height:${Math.max(x.revenue?8:0,(x.revenue/maxRevenue)*100)}%"></div></div><span>${x.label}</span></div>`).join('')}</div></div>
