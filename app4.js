...
function renderAdmin(){
 ...
 document.getElementById('adminBookings').innerHTML=bookings.length?bookings.map(b=>`<div class="card" style="margin-bottom:12px"><div style="display:flex;justify-content:space-between;gap:12px"><div><strong>${fmtDate(b.date)} • ${b.time}</strong><div>${b.clientName}</div><div class="muted">${b.serviceName} • ${b.professionalName}</div><div class="muted">WhatsApp: ${b.phone}</div>${b.status==='proposta'&&b.proposedDate?`<div class="muted top-gap"><strong>Sugestão enviada:</strong> ${fmtDate(b.proposedDate)} às ${b.proposedTime}</div>`:''}</div><div style="text-align:right"><strong>${money(b.price)}</strong><br><span class="pill ${b.status==='confirmado'?'status-confirmado':''}">${statusLabel(b.status)}</span></div></div>${b.status==='pendente'?`<div class="top-gap" style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn primary" onclick="setBookingStatus('${b.id}','confirmado')">Confirmar</button><button class="btn" onclick="suggestAlternative('${b.id}')">Sugerir outro horário</button><button class="btn danger" onclick="setBookingStatus('${b.id}','recusado')">Recusar</button></div>`:''}${b.status==='proposta'?`<div class="top-gap" style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn primary" onclick="setBookingStatus('${b.id}','confirmado')">Confirmar mesmo assim</button><button class="btn" onclick="suggestAlternative('${b.id}')">Alterar sugestão</button></div>`:''}</div>`).join(''):'<div class="card empty">Nenhum agendamento cadastrado.</div>';
}
